...
function showEquipmentPopover(trigger, shouldPin) {
    if (!trigger) {
        return;
    }
    ...
    if (equipmentPopoverCredit) {
        equipmentPopoverCredit.textContent = '';
        equipmentPopoverCredit.classList.add('hidden');
    }
...
function hideEquipmentPopover(immediate = false, force = false) {
    if (!equipmentPopover) {
        return;
    }
...
function initializeEquipmentPopovers(container) {
...
function renderCricothyrotomyDetail(topic, details, contentArea) {
    ...
    const sectionsData = Array.isArray(variantData.sections) && variantData.sections.length
