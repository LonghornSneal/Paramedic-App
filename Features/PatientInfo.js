...
let patientData = {
            age: null, weight: null, weightUnit: 'kg', pmh: [], allergies: [], currentMedications: [], indications: [], symptoms: [],
            vitalSigns: { bp: '', hr: null, spo2: null, etco2: null, rr: null, bgl: '', eyes: '', gcs: null, aoStatus: '', lungSounds: '' }, ekg: '' };
const ptInputIds = [      // IDs of all patient data input fields in the sidebar:
...
function updatePatientData() {      // Read and parse inputs from the sidebar fields
    ...
    hr: document.getElementById('vs-hr').value 
         ? parseInt(document.getElementById('vs-hr').value, 10) : null,
    spo2: document.getElementById('vs-spo2').value 
          ? parseInt(document.getElementById('vs-spo2').value, 10) : null,
    etco2: document.getElementById('vs-etco2').value 
           ? parseInt(document.getElementById('vs-etco2').value, 10) : null,
    rr: document.getElementById('vs-rr').value 
         ? parseInt(document.getElementById('vs-rr').value, 10) : null,
    bgl: document.getElementById('vs-bgl').value.trim(), eyes: document.getElementById('vs-eyes').value.trim(),
    gcs: document.getElementById('vs-gcs').value 
          ? parseInt(document.getElementById('vs-gcs').value, 10) : null,
  ...
  patientData.ekg = document.getElementById('pt-ekg').value.trim(); }
    ...
    // Filter the medication/topic list based on selected indications
      ...
      const medIndications = med.details.indications.map(i => i.toLowerCase());
                ...
                } else {     // No patient indications given or no indications data on this topic – ensure it's not struck out
  ...
  if (currentTopicTitleEl) {
    ...
    if (currentTopicId && allDisplayableTopicsMap[currentTopicId]) {
      // Re-render the detail page for the current topic without altering history or scroll
    ...
    if (patientData.age !== null) {
      if (patientData.age >= PEDIATRIC_AGE_THRESHOLD) {
        // Patient is an adult – strike out pediatric dose sections
        ...
        // Patient is pediatric – strike out adult dose sections
