#!/usr/bin/env node
// Lightweight MCP server exposing local dev helpers (CSS, preview, verify)
const { Server } = require("@modelcontextprotocol/sdk/server/index.js");
const { StdioServerTransport } = require("@modelcontextprotocol/sdk/server/transport/stdio.js");
const path = require("node:path");
const fs = require("node:fs/promises");
const { spawn } = require("node:child_process");

// Lazy-load Playwright to speed startup when not needed
let _playwright;
async function pw() {
  if (!_playwright) {
...
function startServer() {
  ...
  server.tool(
    {
      name: "start_preview",
      description:
        "Start a simple static HTTP server for the app and return the URL.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  server.tool(
    {
      ...
      description:
        "Open a page (local path or URL) and capture a screenshot of the page or element.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  // Tool: computed_styles
  server.tool(
    {
      name: "computed_styles",
      description:
        "Get computed CSS styles for a selector on a given page (URL or local).",
      inputSchema: {
        type: "object",
        properties: {
          ...
          properties: {
            type: "array",
            items: { type: "string" },
            description: "Optional list of CSS properties to return",
          },
        },
        required: ["selector"],
      },
  ...
  server.tool(
    {
      name: "verify_dom",
      description:
        "Verify DOM conditions: text presence, visibility, and optional computed style equals.",
      inputSchema: {
        type: "object",
        properties: {
          target: { type: "string", default: defaultIndex },
            ...
            items: {
              ...
              properties: {
                ...
                styleEquals: {
                  ...
                  description: "Map of CSS property -> expected value",
                },
              },
              required: ["selector"],
            },
  ...
  // Tool: append_css_block
  server.tool(
    {
      name: "append_css_block",
      description:
        "Append a CSS rule block to styles.css (or specified file). This is additive and safe.",
      inputSchema: {
        type: "object",
        properties: {
          selector: { type: "string" },
          declarations: {
            type: "array",
            items: { type: "string" },
            description: "Array of CSS declarations like 'color: red'",
          },
  ...
  server.tool(
    {
      name: "run_linters",
      description:
        "Run project linters (stylelint, csstree-validator, html-validate, eslint, webhint).",
      inputSchema: { type: "object", properties: {} },
    },
    async () => {
  ...
  // Tool: scan_unused_code
  server.tool(
    {
      name: "scan_unused_code",
      description:
        "Scan for unused exports/files and dependencies using knip and depcheck.",
      inputSchema: { type: "object", properties: {} },
    },
    async () => {
