...
**[2025-10-21 Patient sidebar per-field CSS split: Migrated the monolithic patient-sidebar-forms.css into 20 per-category modules (Sex → EKG Modifiers), converted shared helper rules into data-line scoped grid layouts, and deprecated patient-sidebar-controls.css in favor of the new manifest (files: styles/sidebar/patient-sidebar-forms.css; styles/sidebar/patient-sidebar-forms/**.css; styles/sidebar/patient-sidebar-controls.css; README.md). Tests: npm run lint.]**
**[2025-10-21 Patient sidebar select chevron toggle: Added runtime detection so patient sidebar selects drop their chevron after a choice, introduced per-field rules to clear the arrow when populated, and documented each sidebar CSS module with scope comments (files: Features/patient/PatientSidebar.js; styles/sidebar/patient-sidebar-forms.css; styles/sidebar/patient-sidebar-forms/**.css; styles/sidebar/patient-sidebar-controls.css). Tests: npm run lint (timeout after existing webhint errors surfaced).]**
